...
from app.models import SessionLocal, Jogo, Serie, Filme
...
def buscar_filme(genero= None, ano = None, idioma = None):
    session = SessionLocal()
    query = session.query(Filme)
    
    if genero:
        codigo_genero = lista_de_generos_filmes.get(genero)
        if codigo_genero:
            query = query.filter(Filme.genres.contains([codigo_genero]))
            
    if ano:
        query = query.filter(extract('year', Filme.release_date) == int(ano))
        
    if idioma:
        codigo_idioma = lista_de_idiomas_filmes.get(idioma)
        if codigo_idioma:
            query = query.filter(Filme.original_language == codigo_idioma)

            
    filmes_filtrados = query.all()
    session.close()
